...
def test_artist_signup_serializer_ko(foobar):
    ...
    assert "Passwords mismatch" in str(serializer.errors["non_field_errors"][0])

    data["email"] = "foobar@test.com"
    data["password2"] = "pwd123"
    serializer = serializers.SignupArtistSerializer(data=data)
    assert not serializer.is_valid()
    assert "This email address is already used" in str(serializer.errors["email"][0])
    ...
    assert not serializer.is_valid()
    assert "username must contain only letters" in str(serializer.errors["username"][0])

    data["username"] = "foobar"
    serializer = serializers.SignupArtistSerializer(data=data)
    assert not serializer.is_valid()
    assert "This username is already used" in str(serializer.errors["username"][0])
...
def test_artist_signin_serializer_ok(foobar):
    data = {"username": "foobar", "password": "testpwd123"}
...
def test_artist_signin_serializer_ko(foobar):
    data = {"username": "keser", "password": "wrongpwd123"}
    serializer = serializers.SigninArtistSerializer(data=data)
    ...
    assert not serializer.is_valid()
    assert "invalid password" in str(serializer.errors["non_field_errors"][0])
...
def test_artist_model_serializer_update(foobar):
    user = User.objects.create(username="dummy", email="dummy@test.com")
